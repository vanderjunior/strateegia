...
This repository is best described as a local/private alpha. It is suitable for one student to experiment with textual materials using grounded summaries and validated objective questions when evidence is sufficient. Explicit answer review now persists immutable, owner-scoped attempts, but those attempts do not yet change question selection. It is not yet safe to rely on Mentorium as the primary preparation environment for a real exam until adaptation, OCR/corpus coverage, question quality, review execution, backup procedures, and production persistence are hardened.
...
- No adaptive queue, weak-topic scheduling, question suppression, Leitner buckets, or mastery state.
...
| Adaptive selection | `MISSING` | No adaptive queue | Attempts do not suppress, prioritize, or reschedule questions yet. |
...
- Answer attempts are persisted, but they do not yet alter question order or future eligibility.
