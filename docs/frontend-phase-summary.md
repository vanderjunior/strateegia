...
- Backend `GET /api/study/review/next` now provides a read-only cumulative-review candidate based on prepared materials or available study blocks; frontend proxy/UI are pending, and `studied`, `completed`, attempts, scoring, and progress mutation remain reserved for a future Progress phase.
