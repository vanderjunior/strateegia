...
- Selectable answer UI planning is documented in `docs/answer-review-selectable-ui-plan.md`: the next UI phase should add radio-based review for `multiple_choice` and `true_false` only, use the existing answer-review helper, and keep gabarito, correct alternatives, scoring, persistence, progress, simulado, OCR, and LLM behavior out of scope.
