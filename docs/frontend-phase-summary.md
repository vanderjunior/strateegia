...
- Progress semantics planning is documented in `docs/progress-semantics-contract-plan.md`; backend `POST /api/study/progress/events` and `GET /api/study/progress/summary` now exist for explicit user-scoped progress events. Frontend proxy/UI remain pending, and `studied_materials`, `completed`, persisted attempts, scoring, and progress-aware review-after-3 remain future-only.
...
- Progress semantics are backend-only in `docs/progress-semantics-contract-plan.md` and `docs/read-endpoints-contract.md`; there is no frontend proxy, `Marcar como estudado` UI, persisted attempt/correction history, studied/completed material state, or progress-aware review-after-3 behavior yet.
