...
- Study blocks have a minimal `/study` list UI, a minimal `/study/blocks/[blockId]` detail UI, and a review-only fixation-question card. There is still no answer input, answer correction, review-after-3 behavior, progress mutation, generated questions, simulado, OCR, or LLM behavior.
...
- Answer review planning is documented in `docs/answer-review-contract-plan.md`: the preferred future endpoint is `POST /api/study/blocks/{block_id}/questions/{question_id}/answer/review`, with bounded feedback and no progress mutation in the first implementation.
