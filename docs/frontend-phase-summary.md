...
- Progress semantics planning is documented in `docs/progress-semantics-contract-plan.md`: current study/review surfaces remain read-only or stateless, while `studied`, `completed`, progress updates, persisted attempts, scoring, and progress-aware review-after-3 require a future explicit Progress phase.
...
- Progress semantics are planning-only in `docs/progress-semantics-contract-plan.md`; there is no `Marcar como estudado` UI, progress write endpoint, persisted attempt history, studied/completed material state, or progress-aware review-after-3 behavior yet.
