...
- Progress-aware review eligibility planning is documented in `docs/review-block-progress-contract-plan.md`: review-after-3 remains based on prepared materials/study blocks today, `studied_blocks_count` does not imply studied materials, `studied_materials_count` remains `0`, and future `review_basis=studied_materials` must be backend-derived conservatively after material-studied semantics are implemented.
