...
- Cumulative review planning is documented in `docs/review-block-contract-plan.md`: the first review-after-3 contract should be read-only and based on prepared materials or available study blocks, while `studied`, `completed`, attempts, scoring, and progress mutation remain reserved for a future Progress phase.
