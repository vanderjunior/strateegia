...
- Cumulative review card QA is closed through Compose/API plus hydrated UI tests: after rebuilding stale frontend/backend containers, authenticated `/api/study/review/next` moved from `not_ready` with 2 prepared materials to `needs_review` after adding/preparing one local QA study material, and the UI contract remains read-only without exposing `/study/review/<id>`, gabarito, score, progress, simulado, raw content, or internal traces.
- Progress semantics planning is documented in `docs/progress-semantics-contract-plan.md`; backend and frontend same-origin `POST /api/study/progress/events` / `GET /api/study/progress/summary` plus `createStudyProgressEvent()` / `fetchStudyProgressSummary()` now exist for explicit user-scoped progress events. `/study/blocks/[blockId]` now has a minimal explicit `Marcar bloco como estudado` button that posts `block_marked_studied` only after user click. Automatic page-view tracking, material completion, persisted attempts, scoring, and progress-aware review-after-3 remain future-only.
- `/study` now shows a compact read-only `Acompanhamento do estudo` card from `fetchStudyProgressSummary()`. It displays explicit bounded counts only, such as prepared materials, blocks marked as studied, and unscored reviewed questions; it does not create progress events, infer material completion, add scoring/correction, or make cumulative review eligibility progress-aware.
- Progress-Summary-QA-A browser/API smoke validated the read-only card in Compose: opening `/study` showed the progress summary card, did not change progress counts, clicking `Marcar bloco como estudado` from one block detail moved `studied_blocks_count` from `2` to `3`, and returning to `/study` showed the updated count while `prepared_materials_count=3`, `studied_materials_count=0`, and `review_basis=prepared_materials` stayed conservative. A tiny copy fix renamed the disabled sidebar item from `Simulados` to `Avaliações` to avoid implying an executable simulado surface on study pages.
- Progress-aware review eligibility is documented in `docs/review-block-progress-contract-plan.md`: `studied_blocks_count` does not by itself imply studied materials, and `review_basis=studied_materials` must be backend-derived conservatively from explicit studied-block events.
- Backend conservative studied-material derivation now exists: a prepared `study_material` counts in `studied_materials_count` only when all of its backend-derived blocks have explicit `block_marked_studied` events. `GET /api/study/review/next` returns `basis=studied_materials` only at 3 or more such materials; otherwise prepared-material review remains the fallback. No frontend UI/copy change, material completion, percentages, scoring, correction, gabarito, or simulado behavior was added.
- Frontend review/progress API and `/study` copy now accept backend-provided `studied_materials`: review cards can show `Baseada em materiais estudados`, and the progress card can show `Revisão sugerida com base em materiais estudados.` plus `Materiais estudados` only when the backend returns those values. The frontend still does not infer studied materials from block counts and adds no material completion UI, percentages, scoring, correction, gabarito, or simulado behavior.
- ReviewBlock-Progress-QA-A Compose/API smoke validated the progress-aware basis after rebuilding frontend and backend containers: the seeded user moved from `studied_materials_count=2` / `review_basis=prepared_materials` to `studied_materials_count=3` / `review_basis=studied_materials` after explicit `block_marked_studied` events for all visible study blocks. `GET /api/study/review/next` then returned `basis=studied_materials`, repeated reads were stable, and serialized responses exposed no raw content, storage paths, answer keys, gabarito, score, correction, progress internals, or material-completion wording. Browser `/study` remained behind the current edital-analysis gate for this dataset, so hydrated studied-material copy remains covered by frontend tests until a browser fixture has both analyzed edital access and three conservatively studied materials.
- ReviewBlock-Progress-Fixture-A adds a deterministic development/test-only QA fixture for browser validation of the studied-material review basis. The explicit seed creates or reuses the dedicated Compose QA user, one bounded analyzed edital, three prepared study materials, and idempotent `block_marked_studied` events for each backend-derived block so `/api/study/progress/summary` returns `review_basis=studied_materials` and `/api/study/review/next` returns `basis=studied_materials`. It is not production startup behavior and adds no frontend product changes, material completion, percentages, score, gabarito, correction, simulado, OCR, LLM, scheduler, PostgreSQL, provider, or signup behavior.
- ReviewBlock-Progress-QA-B closes the studied-material visual copy check with the explicit fixture. Compose/API returned `studied_materials_count=5`, `review_basis=studied_materials`, review `basis=studied_materials`, and frontend-safe fixture blocks. Browser `/study` rendered `Seu caminho de estudo`, `Revisão acumulada sugerida`, `Baseada em materiais estudados`, `Acompanhamento do estudo`, `Materiais estudados`, and `Revisão sugerida com base em materiais estudados.` with no `/study/review/<id>` link and no forbidden completion, score, gabarito, correction, simulado, raw-content, or internal-copy terms. A tiny `/study` copy fix changed the progress-card label to `Questões revisadas nesta etapa` to avoid visible `pontuação` wording on this page.
- UX-Polish-A documents the current product-surface inventory and simplification plan in `docs/ux-polish-product-surface-plan.md`. The audit found the core study path functionally complete enough for behavior-preserving polish: next work should reduce visual density, repeated explanatory copy, navigation/status noise, and competing cards before adding new study behavior.
- UX-Polish-C refined `/study` hierarchy and density without changing study/review/progress behavior. The page now promotes a single `Continue seus estudos` action for the first backend-ordered block, keeps `Seu caminho de estudo` as the primary block list, and makes `Revisão acumulada` plus `Acompanhamento do estudo` compact secondary sections. Non-actionable review/progress states no longer compete with the main path when blocks are available.
- Progress-QA-A browser/API smoke validated the explicit block study progress happy path in Compose after rebuilding stale frontend/backend containers: page load did not change `opened_blocks_count` or `studied_blocks_count`, clicking `Marcar bloco como estudado` showed success copy, and `studied_blocks_count` moved from `0` to `1` while `studied_materials_count` stayed `0`. Repeating the same `block_marked_studied:<block_id>` idempotency key did not duplicate the count. Progress-C-Fix-A resolved the invalid-request follow-up: the frontend progress-events proxy no longer coerces unknown or missing `event_type` / `target_type` values into defaults, and invalid progress payloads return `422`.
- Progress-QA-B Compose/API smoke validated the fixed frontend proxy path after rebuilding/recreating the frontend: invalid `event_type`, invalid `target_type`, and missing `event_type` / `target_type` / `target_id` each returned `422` with no `event_id` and left `opened_blocks_count`, `studied_blocks_count`, `reviewed_questions_count`, `prepared_materials_count`, and `studied_materials_count` unchanged. A valid `block_marked_studied` event with unsafe extra fields returned only bounded event metadata, incremented `studied_blocks_count` once, and the repeated idempotency key did not duplicate the count.
...
- Left navigation now separates available areas from gated/future areas: Ciclo waits for analyzed edital, while Questões, Avaliações, and Execução are marked as preparation/unavailable instead of looking active.
...
- Progress semantics are documented in `docs/progress-semantics-contract-plan.md` and `docs/read-endpoints-contract.md`; block detail now has explicit `Marcar bloco como estudado` UI, but there is still no automatic page-view tracking, persisted attempt/correction history, material completion state, score, or progress-aware review-after-3 behavior.
...
- Ciclo, Questões, Avaliações, Execução, automatic/material-level progress, and multi-material study cycles are not real user capabilities yet; they remain gated or future placeholders until later contracts exist.
...
6. UX-Polish-D: behavior-preserving block-detail hierarchy and post-answer grouping refinement.
