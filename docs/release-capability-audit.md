...
- Selected answers and attempt numbers now survive restart; Adaptive-Queue-A adds a deterministic backend queue derived from those attempts, and Personal-Study-E2E-A connects block detail to that queue.
- The queue prioritizes eligible prior errors and ungraded questions, preserves new current-block questions, suppresses recent correct answers by attempt milestones, and includes a bounded historical sample from explicitly studied blocks.
...
| --- | --- | --- |
| A. Local personal alpha | `GO` | Grounded summaries, validated objective questions, persisted attempt history, block-detail attempt-aware queue consumption, and Compose textual E2E evidence now exist. Keep backups and manually review deterministic outputs. |
...
| D. Adaptive question memory | `CONDITIONAL_GO` | Block detail consumes the backend-derived queue and attempts survive restart, but no full cyclical scheduler or executable review session exists. |
...
The current product can ingest an edital, prepare textual materials, show an ordered study path, expose extractive source-grounded summaries/key points, show deterministic objective questions with internal correctness when validated, remember explicit selected-answer attempts, derive a bounded attempt-aware backend queue, record block-study progress, and show a cumulative review candidate. It still cannot prove syllabus sufficiency, execute a full cumulative review session, handle OCR-required corpora by default, or support public production.

## GO / NO-GO Verdicts

| Scenario | Verdict | Conditions |
| --- | --- | --- |
| A. Local personal alpha | `GO` | Good for eligible textual `.md`, `.txt`, and textual `.pdf` content, while reviewing deterministic outputs and keeping backups. |
...
| D. Adaptive question memory | `CONDITIONAL_GO` | Block detail consumes the milestone-based backend queue; full cyclical scheduling remains incomplete. |
...
| Study blocks | `USABLE_ALPHA` | `routes.py` helpers | `GET /api/study/blocks`, `/study/session/next` | `/study` | Computed from prepared summaries | Study block/session tests | 3 blocks generated | Main path is ordered list; adaptive queue affects block-detail questions only | Real cycle contract |
...
| Fixation questions | `USABLE_ALPHA` | `_internal_fixation_question_candidates`, `_bounded_fixation_questions_response`, `_bounded_adaptive_question_queue_response` | `GET /api/study/blocks/{id}/questions`, `GET /api/study/blocks/{id}/questions/next` | Block detail consumes the adaptive queue through a same-origin proxy | Deterministically computed; queue derived on read from persisted attempts | Grounded fixation, fixation read, answer-review, attempt-memory, adaptive-queue, and frontend queue tests | A-E/A-D and C/E questions use bounded source evidence and stable fingerprints; queue prioritizes weak/ungraded/new/reviewing/historical items | Conservative generation returns fewer/no questions; not a full scheduler | Full Compose personal-study E2E |
...
| Attempt memory | `USABLE_ALPHA` | `record_study_question_attempt`, bounded history helper | Answer review POST; `GET /api/study/blocks/{block_id}/questions/{question_id}/attempts` | Submission is connected; queue refresh is connected; history remains API-only | Selected answer, correctness state, identity/version, context, and attempt number persist | `test_question_attempt_memory_api.py`, `test_adaptive_question_queue_api.py`, block-detail queue tests | Exact retry is stable; conflicting key reuse returns 409; new keys allow later attempts; reload preserves history; queue derives state without writing | Single-replica JSON; no history dashboard | Personal-study E2E |
...
The history is immutable, owner-scoped, bounded to the latest 20 records per resolved question, and survives repository reload.

Adaptive-Queue-A derives question state on read without mutating attempts:

- `new`: no attempts exist.
- `ungraded`: latest attempt is ungraded.
- `weak`: latest validated attempt is incorrect.
- `reviewing`: latest validated attempt is correct with fewer than two consecutive correct attempts.
- `temporarily_mastered`: at least two consecutive validated correct attempts, never permanent.

Eligibility uses later explicit attempts by the same user, not fixed wall-clock intervals: weak/ungraded after 1 later attempt, reviewing after 3, and temporarily mastered after 8 or as a bounded historical sample. The queue is deterministic and owner-scoped, avoids immediate repetition when alternatives exist, samples historical questions only from explicitly studied blocks, and now powers the block-detail fixation-question order. It is still not a complete cyclical scheduler.
...
- Result after Adaptive-Queue-A: `1556 passed`, `0 failed`, `5 existing SWIG/PyMuPDF deprecation warnings`.
- Focused adaptive/attempt/question/review suite: `54 passed`.
...
- Runtime queue smoke: five validated questions across current/studied blocks, weak question prioritized after an intervening attempt, reviewing/mastered cooldowns honored by attempt milestones, bounded historical item present, queue reload identical, and queue GET created no attempts or progress events.

Frontend:

- Focused queue/block-detail suite: `7 passed`, `100 tests`.
- `npm run test`: `59 passed`, `523 tests`.
- `npm run build`: passed.
- `npm run typecheck`: passed after the build.

Compose Personal-Study-E2E-A:

- `docker compose build` and `docker compose up -d` succeeded; backend `/api/exam-profiles` and frontend `/` returned `200`.
- The explicit development fixture was run twice and remained idempotent for studied-material review semantics.
- A fresh E2E user uploaded and analyzed one textual edital, uploaded and prepared three Markdown study materials, received three study blocks, opened `/study`, and opened a block with grounded summary/key points plus an adaptive queue.
- Representative block evidence: summary `ready`, four source-backed key points, three public fixation questions, no public answer key, no score, and no mastery fields.
- Answer submission evidence: one question accepted alternatives `A` and `B` as server-derived `incorrect`, `C` as server-derived `correct`; exact retry returned the same attempt, conflicting idempotency reuse returned `409`, and a later explicit attempt was allowed.
- After explicit block-study events for all three blocks, progress returned `prepared_materials_count=3`, `studied_materials_count=3`, `studied_blocks_count=3`, and `review_basis=studied_materials`; cumulative review returned `basis=studied_materials`.
- After backend restart, login succeeded, progress/review state persisted, the adaptive queue remained available, and backend attempt history returned the persisted attempts in stable order.
- A second user had zero study blocks and zero studied blocks, confirming owner isolation for the sampled E2E path.
