...
export async function loginWithPassword({
  username,
  password
}: LoginWithPasswordInput): Promise<ApiResult<BackendCurrentSession>> {
  const { baseUrl, forceMock } = getApiConfig();

  if (forceMock) {
    return makeApiFailure("mock", "mock_mode", "Modo de demonstração: login real indisponível.");
  }

  if (!baseUrl) {
    return makeApiFailure("unsupported", "missing_base_url", "Sessão não configurada neste ambiente.");
  }

  try {
    const response = await fetch(LOGIN_PROXY_PATH, {
      method: "POST",
      headers: {
        Accept: "application/json",
    ...
    if (response.status === 401) {
      return makeApiFailure("backend", "invalid_credentials", "Credenciais inválidas.", 401);
    }
    if (response.status === 400 || response.status === 422) {
      return makeApiFailure("backend", "validation_failed", "Preencha usuário e senha para entrar.", response.status);
    }
    if (!response.ok) {
      return makeApiFailure("backend", "http_error", `Backend returned HTTP ${response.status}.`, response.status);
    }
...
export async function logoutCurrentSession(): Promise<ApiResult<BackendCurrentSession>> {
  const { baseUrl, forceMock } = getApiConfig();

  if (forceMock) {
    return makeApiFailure("mock", "mock_mode", "Modo de demonstração: logout real indisponível.");
  }
  ...
  try {
    const response = await fetch(LOGOUT_PROXY_PATH, {
      method: "POST",
      headers: {
        Accept: "application/json"
      },
