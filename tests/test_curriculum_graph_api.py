...
def test_curriculum_graph_endpoints_work_for_owner_and_are_json_safe(tmp_path):
    ...
    build = owner.post(f"/api/edital/{edital_id}/curriculum-graph/build")
    loaded = owner.get(f"/api/edital/{edital_id}/curriculum-graph")
    graph_id = loaded.json()["graph_id"]
    by_id = owner.get(f"/api/curriculum-graph/{graph_id}")

    assert build.status_code == 200
    assert loaded.status_code == 200
    assert by_id.status_code == 200
    assert loaded.json()["graph_id"] == graph_id
    ...
    unauth = anonymous.post(f"/api/edital/{edital_id}/curriculum-graph/build")
    assert unauth.status_code == 401


def test_non_owner_cannot_access_other_user_curriculum_graph(tmp_path):
    ...
    build = owner.post(f"/api/edital/{edital_id}/curriculum-graph/build")
    assert build.status_code == 200
    graph_id = build.json()["graph_id"]

    assert other.post(f"/api/edital/{edital_id}/curriculum-graph/build").status_code == 404
    assert other.get(f"/api/edital/{edital_id}/curriculum-graph").status_code == 404
    assert other.get(f"/api/curriculum-graph/{graph_id}").status_code == 404
