...
def prepare_question_generation_blueprint(client: TestClient) -> str:
    ...
    simulado = client.post(f"/api/study-cycle/{cycle_id}/simulado-blueprint/build")
    assert simulado.status_code == 200
    blueprint = client.post(
        f"/api/simulado-blueprint/{simulado.json()['blueprint_id']}/question-generation-blueprint/build"
    )
    assert blueprint.status_code == 200
    return blueprint.json()["blueprint_set_id"]


def test_question_draft_generation_endpoints_work_for_owner_and_are_json_safe(tmp_path):
    owner, _, anonymous, _ = create_clients(tmp_path)
    register_and_login(owner, "owner")
    blueprint_set_id = prepare_question_generation_blueprint(owner)
    ...
    assert loaded.json()["source_question_generation_blueprint_set_id"] == blueprint_set_id
    assert loaded.json()["no_final_question_generated"] is True
...
def test_question_draft_generation_build_is_deterministic_and_does_not_duplicate(tmp_path):
    owner, _, _, repository = create_clients(tmp_path)
    owner_user_id = register_and_login(owner, "owner")
    blueprint_set_id = prepare_question_generation_blueprint(owner)

    first = owner.post(f"/api/question-generation-blueprint/{blueprint_set_id}/question-drafts/build")
    second = owner.post(f"/api/question-generation-blueprint/{blueprint_set_id}/question-drafts/build")
    listed = repository.list_user_question_draft_sets(user_id=owner_user_id)
...
def test_non_owner_cannot_access_other_user_question_draft_set(tmp_path):
