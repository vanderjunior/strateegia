...
def test_controlled_mutation_commit_shell_does_not_leak_sensitive_or_answer_key_content(tmp_path):
...
def test_controlled_mutation_commit_shell_build_and_get_do_not_mutate_source_artifacts(tmp_path):
    repository = JsonStudyRepository(tmp_path / "study_data.json")
    fixture = api_readonly_fixture(tmp_path, repository=repository)
    result = build_controlled_mutation_commit_shell(fixture)
    assert result is not None
    ...
    service = SimuladoControlledRuntimeMutationCommitService(repository)
