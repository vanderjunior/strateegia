...
def test_minimal_progress_ledger_apply_does_not_leak_sensitive_or_answer_key_content(tmp_path):
    repository = JsonStudyRepository(tmp_path / "study_data.json")
    fixture = allowed_minimal_progress_ledger_apply_fixture(tmp_path, repository=repository)
...
def test_minimal_progress_ledger_apply_build_and_get_do_not_mutate_source_artifacts(tmp_path):
    repository = JsonStudyRepository(tmp_path / "study_data.json")
    fixture = allowed_minimal_progress_ledger_apply_fixture(tmp_path, repository=repository)
    result = build_minimal_progress_ledger_apply(fixture)
    assert result is not None
    ...
    loaded = service.get_minimal_progress_ledger_apply(
        result.source_runtime_apply_policy_id,
        user_id=fixture.context.user_id,
    )
    by_id = service.get_minimal_progress_ledger_apply_by_id(
        result.minimal_progress_ledger_apply_id,
        user_id=fixture.context.user_id,
    )
