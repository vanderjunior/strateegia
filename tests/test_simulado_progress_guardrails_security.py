...
from tests.fixtures.simulado_progress_guardrails import build_progress_guardrail, no_scoreable_items_fixture
from tests.fixtures.simulado_question_assemblies import assembly_json_keys


def test_simulado_progress_guardrails_do_not_leak_sensitive_or_answer_key_content(tmp_path):
    repository = JsonStudyRepository(tmp_path / "study_data.json")
    fixture = no_scoreable_items_fixture(tmp_path, repository=repository)
...
def test_simulado_progress_guardrails_build_and_get_do_not_mutate_source_artifacts(tmp_path):
    repository = JsonStudyRepository(tmp_path / "study_data.json")
    fixture = no_scoreable_items_fixture(tmp_path, repository=repository)
    result = build_progress_guardrail(fixture)
    assert result is not None
    ...
    assert before_score_result is not None
    ...
    assert after_score_result is not None
