...
def test_runtime_progress_application_does_not_leak_sensitive_or_answer_key_content(tmp_path):
...
def test_runtime_progress_application_build_and_get_do_not_mutate_source_artifacts(tmp_path):
    repository = JsonStudyRepository(tmp_path / "study_data.json")
    fixture = no_runtime_mutation_fixture(tmp_path, repository=repository)
    result = build_runtime_progress_application(fixture)
    assert result is not None
    ...
    loaded = service.get_application(
        runtime_guardrail.runtime_guardrail_id,
        user_id=fixture.context.user_id,
    )
    by_id = service.get_application_by_id(
