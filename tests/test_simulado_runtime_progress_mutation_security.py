...
def test_runtime_progress_mutation_does_not_leak_sensitive_or_answer_key_content(tmp_path):
    repository = JsonStudyRepository(tmp_path / "study_data.json")
    fixture = no_runtime_mutation_fixture(tmp_path, repository=repository)
    result = build_runtime_progress_mutation_transaction(fixture)
...
def test_runtime_progress_mutation_build_and_get_do_not_mutate_source_artifacts(tmp_path):
    repository = JsonStudyRepository(tmp_path / "study_data.json")
    fixture = no_runtime_mutation_fixture(tmp_path, repository=repository)
    source_explicit = fixture.explicit_apply
    assert source_explicit is not None
    result = build_runtime_progress_mutation_transaction(fixture)
    assert result is not None
    service = SimuladoRuntimeProgressMutationService(repository)

    before_explicit = repository.get_simulado_explicit_runtime_apply_by_id(
        source_explicit.explicit_apply_id,
        user_id=fixture.context.user_id,
    )
    before_shell = repository.get_simulado_controlled_apply_shell_by_id(
        source_explicit.source_apply_shell_id,
        user_id=fixture.context.user_id,
    )
    before_application = repository.get_simulado_runtime_progress_application_by_id(
        source_explicit.source_application_id,
        user_id=fixture.context.user_id,
    )
    before_runtime_guardrail = repository.get_simulado_runtime_guardrail_by_id(
        source_explicit.source_runtime_guardrail_id,
        user_id=fixture.context.user_id,
    )
    before_integrated = repository.get_simulado_integrated_result_by_id(
        source_explicit.source_integrated_result_id,
        user_id=fixture.context.user_id,
    )
    before_progress = repository.load_progress(user_id=fixture.context.user_id)

    loaded = service.get_mutation_transaction(
        source_explicit.explicit_apply_id,
        user_id=fixture.context.user_id,
    )
    by_id = service.get_mutation_transaction_by_id(
        result.mutation_transaction_id,
        user_id=fixture.context.user_id,
    )
